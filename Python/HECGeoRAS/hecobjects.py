...
class HecRasObject(object):
    ...
    def build_table_sql(self):
        fields = ['id serial PRIMARY KEY', 'geom geometry({0}, {1})'.format(self.geom_type, self.srid)]
        fields += [' '.join(field) for field in self.attrs]
    ...
    def get_length(self):
        qry = '''
        UPDATE "{0}"."{1}"
            SET "ArcLength" = ST_Length("{2}"."{3}".geom)'''.format(self.schema, self.name, self.schema, self.name)
        self.sql['length'] = qry
