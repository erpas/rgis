...
class DlgRasWaterSurfaceGeneration(QDialog):
    def __init__(self, parent=None):
        ...
        self.ui.cboPoints.currentIndexChanged.connect(self.cboPointsChanged)
        self.ui.engineQgisBtn.toggled.connect(self.updateInterpTypeCbo)
    ...
    def rejectDialog(self):
        self.rgis.addInfo('  Loading max WSEL failed or was cancelled.')
    ...
    def acceptDialog(self ):
        ...
        if not self.ptsLayer: # no src layer
            self.rgis.addInfo('  No source points. Cancelling...')
            return
        ...
        if qgisEngine:
            ...
            try:
                ...
                self.rgis.addInfo('  Scipy Python package not installed. Use OSGeo4W installer in advanced mode or execute <pre>pip install scipy</pre> in OSGeo4W shell.' )
                return
            print self.ui.interpTypeCbo.currentIndex()
            if self.ui.interpTypeCbo.currentIndex() == 0:
                intMethod = 'linear'
        ...
        if qgisEngine:
            ...
            try:
                ...
                self.rgis.addInfo('  Memory error! Try to generate smaller raster.')
                return
            except:
                self.rgis.addInfo('  Traceback:<br>%s' % traceback.format_exc())
                return
        ...
        self.rgis.addInfo('  Water surface raster created and saved to a temporary file:\n    %s' % resRasterFilename)
